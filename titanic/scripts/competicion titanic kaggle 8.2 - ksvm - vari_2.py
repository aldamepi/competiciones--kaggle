...
##### Le quito la variable adulto y cubierta
X = df.iloc[:, 1:9]
y = df.iloc[:, 0].values
X.drop("cubierta", axis=1, inplace=True)

x_t = dt.copy()
x_t = x_t.iloc[:, 0:8]
x_t.drop("cubierta", axis=1, inplace=True)
...
outKsvm.to_csv('../datasets/subm_am_py_ksvm_scub_100821.csv')
