...
Cambiar las etiquetas de la varible genero
```{r codificarFactores}
df$genero = factor(df$genero, labels = c(1,2))
df$Puerto = factor(df$Puerto, labels = c(1,2,3,4))

# Codificar factores del cojunto de test
test$Sex = factor(test$Sex, labels = c(1,2))
test$Embarked = factor(test$Embarked, labels = c(2,3,4))
levels(test$Embarked) = c(2,3,4,1)
# test$Embarked = factor(test$Embarked, levels = c(4,3,2,1), labels = c(4,3,2,1))
```
