...
##### Le quito la variable adulto
X = df.iloc[:, 1:9]
y = df.iloc[:, 0].values
X.drop(["cubierta","tarifa"], axis=1, inplace=True)


x_t = dt.copy()
x_t = x_t.iloc[:, 0:8]
x_t.drop(["cubierta","tarifa"], axis=1, inplace=True)
...
dummi_col.remove("adulto")  
dummi_col.remove("cubierta")
...
###### 0.8137512846865365
###### 0.8135817805383023 ( sin adulto )
###### 0.8208281573498966 (sin cubierta)
###### 0.8237267080745342 (sin tarifa)
...
###### 0.8571428571428571
###### 0.8571428571428571 (sin adulto)
###### 0.8514285714285714 (sin cubierta ronda 1)
###### 0.8514285714285714 (sin tarifa)
...
parametersKsvm = [
 #   {'C': [0,5, 1, 10, 100, 1000],'kernel': ['linear']},
              {'C': [3,4,5,6,7],
               'kernel': ['rbf'], 
               'gamma': [0.03,0.05, 0.07,0.1, 0.15,0.2]}
              ]
...
###### 0.8137512846865365
###### 0.8064935064935065
###### 0.8194083694083695 (sin cubierta)
###### 0.8237267080745342 (sin tarifa)
...
#outKsvm.to_csv('../datasets/subm_am_py_ksvm_starif_100821.csv')

###### 0.76076
###### 0.77990 (sin variable cubierta)
###### 0.77751 (sin variable tarifa)
