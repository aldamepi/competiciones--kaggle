...
split divide la cadena de caracteres por un elemento
...
X_combi_k = X_combi.loc[:, ["genero","edad","tarifa","clase_1","clase_2","clase_3"]].values


#### Escalar la matriz k
sc_k = StandardScaler()
X_combi_k_sc = sc_k.fit_transform(X_combi_k)
...
y_kmeans = kmeans.fit_predict(X_combi_k_sc)


### Visualización de los clusters
plt.scatter(X_combi_k[y_kmeans == 0, 1], X_combi_k[y_kmeans == 0, 2], s = 50, c = "red", label = "k1")
plt.scatter(X_combi_k[y_kmeans == 1, 1], X_combi_k[y_kmeans == 1, 2], s = 50, c = "blue", label = "k2")
plt.scatter(X_combi_k[y_kmeans == 2, 1], X_combi_k[y_kmeans == 2, 2], s = 50, c = "green", label = "k3")
plt.scatter(X_combi_k[y_kmeans == 3, 1], X_combi_k[y_kmeans == 3, 2], s = 50, c = "cyan", label = "k4")
plt.scatter(X_combi_k[y_kmeans == 4, 1], X_combi_k[y_kmeans == 4, 2], s = 50, c = "magenta", label = "k5")
# plt.scatter(X_k[y_kmeans == 5, 1], X_k[y_kmeans == 5, 2], s = 50, c = "darkorange", label = "k6")
# plt.scatter(X_k[y_kmeans == 6, 1], X_k[y_kmeans == 6, 2], s = 50, c = "purple", label = "k7")
# plt.scatter(X_k[y_kmeans == 7, 0], X_k[y_kmeans == 7, 1], s = 100, c = "pruple", label = "k4")
...
y_kmeans = pd.DataFrame(y_kmeans, columns = ["k"])
y_kmeans["index"] = X_combi.index
y_kmeans.set_index("index", inplace = True)

X_combi = pd.concat([X_combi,y_kmeans], axis = 1)

X_combi = pd.get_dummies(X_combi, prefix="k", columns=["k"])
...
def procesaResultado (classifier, X_train, y_train, scoring='accuracy'):
    ...
    return np.mean(xval)
"""
esto NO lo entiendo. ¿return np.mean(xval)? ¿Qué está devolviendo esta función
...
#df_output.to_csv("../datasets/subm_am_py_RF.8-k_220821.csv")
