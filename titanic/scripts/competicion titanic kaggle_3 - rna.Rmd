...
* Kernel Support Vector Machine (escalar) - hecho
* muestreo de thompson ?
* rna (escalar)
* xgboost - no necesita escalar - hecho
* deep learning?
...
Me encuentro que hay varias librerias que tienen la función del kernel svm. En el caso de e1071 el kernel radial no tiene sentido ser entrenado porque no tiene parametros a tunear. 

OJO, he metido por error la formula de train en kfold y me devuelve mejores predicciones. Voy a probar a hacer un train a ver que pasa. ¿que es lo que me está haciendo?

Vale, aunque no venga en la lista, se puede usar caret::para entrenar modelos propios, al especificar type = "C-clasification", la función train ejecuta otra funcion que se llama getModelInfo para analizar el modelo. No lo veo muy claro.


Si se quiere tunear el kernel habria que usar otra libreria como kernlab, otro día.

OJO, voy a escalar todas las variables excepto la dependiente.
...
La libreria h2o deeplearning hace por defecto, según la documentación, la cross validation
Su pueden tunear parametros? Cuales? Se puede meter en un train?
```{r rna}
#install.packages("h2o")
library(h2o)
h2o.init(nthreads = -1)
classi_rna = h2o.deeplearning(y = "Superviviente",
...
prob_pred_rna = h2o.predict(classi_rna, 
                        newdata = as.h2o(test2))
y_pred_rna = (prob_pred_rna>0.5)
y_pred_rna = as.data.frame(y_pred_rna)
...
#write.csv(out_ksvm, "../datasets/subm_am_ksvm_310721.csv", row.names = FALSE)
# 0.78229
```
...
```{r outputRNA}
# Pasar de factor a vector:
# out_vect_rna = as.numeric(levels(y_pred_rna)[as.integer(y_pred_ksvm)])
# Crear el dataframe
out_rna = cbind("PassengerId"= test$PassengerId,
               "Survived"= y_pred_rna$p1)
#write.csv(out_rna, "../datasets/subm_am_rna_310721.csv", row.names = FALSE)
# 0.772
# h2o.shutdown()
```
...
Pasos llevados a cabo en R:
    1 Importar Datos:
        ...
        * Traducir nombres de las variables
    ...
    2.2 Ultimar la preparacion de los datos:
        * Tratamiento de los NA
        * Codificar las variables categoricas
